  ...
  return async (c: any, next: any) => {
    try {
      ...
      if (!allowed.includes(profile.role)) {
        console.log(
          `Access denied for user ${userId} with role '${profile.role}'. Required roles: ${allowed.join(", ")}`,
        );
        return c.json(
          {
...
app.post("/make-server-60660975/signup", async (c) => {
  try {
    ...
    sendCredentialsEmail(email, email, password, name, "admin")
...
app.get("/test/schema", requireAuth, async (c) => {
  try {
    ...
    return c.json({
      ...
      sampleColumns:
        data && data.length > 0 ? Object.keys(data[0]) : "No rows to inspect",
...
app.get(
  "/make-server-60660975/organizations",
  requireAuth,
  requireRole(["admin"]),
  async (c) => {
    try {
...
app.post(
  ...
  async (c) => {
    try {
      const { companyName, pan, phoneNo, email, gstNo, address } =
        await c.req.json();
      ...
      if (authErr || !createdUser?.user) {
        console.error(
          "Warning: Failed to create org auth user:",
          authErr?.message,
        );
        ...
        if (profileErr) {
          ...
          try {
            ...
            console.error(
              "Warning: Could not update org with auth details:",
              e,
            );
          }
        }
      ...
      return c.json({
        ...
        organization: {
          ...orgRow,
          auth_user_id: authUserId,
          auth_email: authUserId ? orgAuthEmail : undefined,
        },
        credentials: authCredentials,
        ...(authErr
          ? {
              warning:
                "Organization created but auth setup failed. You can set up login credentials later.",
            }
          : {}),
      });
    } catch (error: any) {
...
// Update organization
app.put(
  "/make-server-60660975/organizations/:id",
  requireAuth,
  requireRole(["admin"]),
  async (c) => {
    try {
...
app.patch(
  "/make-server-60660975/organizations/:id/archive",
  requireAuth,
  requireRole(["admin"]),
  async (c) => {
    try {
...
app.get(
  "/make-server-60660975/organizations/:id/auth",
  requireAuth,
  requireRole(["admin"]),
  async (c) => {
    try {
...
app.post(
  "/make-server-60660975/organizations/:id/auth/reset-password",
  requireAuth,
  requireRole(["admin"]),
  async (c) => {
    try {
      ...
      if (!authUserId) {
        return c.json(
          { error: "Org auth user not created yet. Set email first." },
          400,
        );
      }
...
app.post("/make-server-60660975/devices", requireAuth, async (c) => {
  try {
    const { deviceName, organizationId, model, brandSerialNumber, deviceType } =
      await c.req.json();

    if (!deviceName || !organizationId) {
      return c.json(
        { error: "Device name and organization are required" },
        400,
      );
    }
    ...
    const { data: org, error: orgError } = await supabase
      .from("organizations")
      .select("id, organization_code, email, name")
    ...
    if (brandSerialNumber && brandSerialNumber !== "") {
      ...
      if (existingBrand && existingBrand.length > 0) {
        return c.json(
          { error: "Brand serial number already exists for another device" },
          400,
        );
      }
    }
    ...
    // Send device addition email to organization
    sendDeviceAdditionEmail(
      org.email,
      org.name,
      deviceName,
      ...
      .then(() => console.log(`Device notification email sent to ${org.email}`))
      .catch((err) =>
        console.error(
          `Failed to send device notification email to ${org.email}:`,
          err,
        ),
      );
...
app.put("/make-server-60660975/devices/:id", requireAuth, async (c) => {
  try {
    ...
    if (!deviceName || !organizationId || !serialNumber) {
      return c.json(
        { error: "Device name, organization, and serial number are required" },
        400,
      );
    }
    const supabase = getSupabaseAdmin();
    ...
    if (
      brandSerialNumber !== undefined &&
      brandSerialNumber !== "" &&
      brandSerialNumber !== existingDevice.brand_serial_number
    ) {
      const { data: existingBrand, error: brandErr } = await supabase
        .from("devices")
        .select("id")
      ...
      if (brandErr) {
        console.log(
          "Error checking brand serial uniqueness (update):",
          brandErr,
        );
        return c.json({ error: "Internal server error" }, 500);
      }
    ...
    const updates: any = {
      ...
      brand_serial_number:
        brandSerialNumber !== undefined
          ? brandSerialNumber
          : existingDevice.brand_serial_number,
      model: model || "",
      device_type:
        deviceType !== undefined ? deviceType : existingDevice.device_type,
...
app.patch(
  "/make-server-60660975/devices/:id/archive",
  requireAuth,
  async (c) => {
    try {
      const deviceId = c.req.param("id");
...
// Get all technicians
app.get(
  "/make-server-60660975/technicians",
  requireAuth,
  requireRole(["admin"]),
  async (c) => {
    try {
  ...
  async (c) => {
    try {
      ...
      if (!name || !contactNo || !email || !pan || !aadhar) {
        return c.json(
          { error: "Name, contact, email, PAN and Aadhar are required" },
          400,
        );
      }
      ...
      const { email: techAuthEmail, password: techAuthPassword } =
        computeTechAuth(name);
      ...
      if (authErr || !createdUser?.user) {
        ...
        return c.json(
          { error: authErr?.message || "Failed to create technician user" },
          500,
        );
      }
      ...
      const { error: profileErr } = await supabase
        .from("profiles")
        .upsert(
          { user_id: authUserId, role: "technician", technician_id: techId },
          { onConflict: "user_id" },
        );
      if (profileErr) {
...
// Update technician
app.put(
  "/make-server-60660975/technicians/:id",
  requireAuth,
  requireRole(["admin"]),
  async (c) => {
    try {
...
app.patch(
  "/make-server-60660975/technicians/:id/archive",
  requireAuth,
  requireRole(["admin"]),
  async (c) => {
    try {
...
app.get(
  "/make-server-60660975/technicians/:id/auth",
  requireAuth,
  requireRole(["admin"]),
  async (c) => {
    try {
      ...
      const computedEmail = computeTechAuth(
        String((tech as any).name || ""),
      ).email;
      return c.json({
        technicianId: techId,
        authUserId,
...
app.post(
  "/make-server-60660975/technicians/:id/auth/reset-password",
  requireAuth,
  requireRole(["admin"]),
  async (c) => {
    try {
      ...
      if (!authUserId) {
        return c.json(
          { error: "Technician auth user not created yet. Set email first." },
          400,
        );
      }
...
app.get(
  "/make-server-60660975/maintenance/organization/:orgId",
  requireAuth,
  async (c) => {
    try {
      const supabase = getSupabaseAdmin();
      ...
      if (error) {
        console.error(
          "Error fetching organization maintenance records:",
          error,
        );
        return c.json({ error: "Failed to fetch maintenance records" }, 500);
      }
...
app.get(
  "/make-server-60660975/maintenance/device/:deviceId",
  requireAuth,
  async (c) => {
    try {
      const supabase = getSupabaseAdmin();
...
app.post("/maintenance", requireAuth, async (c) => {
  try {
    const supabase = getSupabaseAdmin();
    const { deviceId, technicianId, organizationId, notes, date, charges } =
      await c.req.json();
    ...
    if (techError || !technicians || technicians.length === 0) {
      return c.json(
        {
          error:
            "Cannot schedule maintenance: No active technicians available. Please add a technician first.",
        },
        400,
      );
    }
    ...
    if (charges !== undefined && charges !== null) {
      ...
      const devType = String(deviceRow.device_type || "")
        .trim()
        .toLowerCase();
      if (devType !== "non comprehensive") {
        return c.json(
          { error: "Charges can only be set for Non Comprehensive devices" },
          400,
        );
      }
    ...
    if (error) {
      ...
      if (
        msg.includes("charges can only be set") ||
        msg.includes("non-comprehensive")
      ) {
        return c.json(
          { error: "Charges can only be set for Non Comprehensive devices" },
          400,
...
app.post("/maintenance/bulk", requireAuth, async (c) => {
  try {
    const supabase = getSupabaseAdmin();
    const { organizationId, deviceIds, technicianId, notes, date } =
      await c.req.json();
    ...
    if (
      ...
      return c.json(
        {
          error: "Organization, device IDs array, and technician are required",
        },
        400,
      );
    }
...
app.patch(
  "/make-server-60660975/tickets/:id/assign",
  requireAuth,
  requireRole(["admin"]),
  async (c) => {
    try {
...
// Admin: update status
app.patch(
  "/make-server-60660975/tickets/:id/status",
  requireAuth,
  requireRole(["admin"]),
  async (c) => {
    try {
      ...
      if (
        !status ||
        !["open", "assigned", "in_progress", "done"].includes(String(status))
      ) {
...
app.get(
  "/make-server-60660975/org/dashboard",
  requireAuth,
  ...
  async (c) => {
    try {
      ...
      if (!organizationId)
        return c.json({ error: "Organization not linked" }, 400);
      ...
      const ticketsAssigned = (tickets || []).filter(
        (t: any) => t.status === "assigned" || t.status === "in_progress",
      ).length;
...
app.get(
  "/make-server-60660975/org/devices",
  requireAuth,
  requireRole(["organization"]),
  async (c) => {
    try {
...
app.get(
  "/make-server-60660975/org/tickets",
  requireAuth,
  requireRole(["organization"]),
  async (c) => {
    try {
...
app.post(
  ...
  async (c) => {
    try {
      ...
      if (device.is_archived === true)
        return c.json(
          { error: "Cannot raise ticket for archived device" },
          400,
        );
...
app.patch(
  ...
  async (c) => {
    try {
      ...
      if (
        !status ||
        !["assigned", "in_progress", "done"].includes(String(status))
      ) {
        return c.json({ error: "Invalid status" }, 400);
      }
      ...
      if (existing.assigned_technician_id !== technicianId)
        return c.json({ error: "Forbidden" }, 403);
...
app.get(
  "/make-server-60660975/org/maintenance",
  requireAuth,
  requireRole(["organization"]),
  async (c) => {
    try {
  ...
  async (c) => {
    try {
      ...
      const deviceIds = Array.from(
        new Set(records.map((r: any) => r.device_id).filter(Boolean)),
      );
      const orgIds = Array.from(
        new Set(records.map((r: any) => r.organization_id).filter(Boolean)),
      );

      // Fetch devices map
      const devicesMap: Record<
        string,
        { name?: string; serial_number?: string; brand_serial_number?: string }
      > = {};
      if (deviceIds.length > 0) {
        ...
        if (devErr) {
          console.log(
            "Warning: failed to fetch devices for tech maintenance:",
            devErr.message,
          );
        } else {
          for (const d of devices || []) {
            devicesMap[d.id as string] = {
              name: d.name,
              serial_number: d.serial_number,
              brand_serial_number: d.brand_serial_number,
            };
          }
        }
      }
      ...
      const orgsMap: Record<
        string,
        { name?: string; organization_code?: string }
      > = {};
      if (orgIds.length > 0) {
        ...
        if (orgErr) {
          console.log(
            "Warning: failed to fetch organizations for tech maintenance:",
            orgErr.message,
          );
        } else {
          for (const o of orgs || []) {
            orgsMap[o.id as string] = {
              name: o.name,
