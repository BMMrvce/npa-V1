...
export const LoginPage: React.FC<LoginPageProps> = ({ onLoginSuccess, variant = 'admin', onBack }) => {
  ...
  const expectedRoleLabel = (v: LoginVariant) => {
    ...
    try {
      if (isLogin) {
        ...
        if (data?.session?.access_token) {
          // Enforce that each login screen only accepts its intended role.
          try {
            const { backendUrl } = await import('../utils/supabase/info');
            const meRes = await fetch(`${backendUrl}/make-server-60660975/me`, {
              headers: { Authorization: `Bearer ${data.session.access_token}` },
            });
            const meJson = await meRes.json().catch(() => ({}));
            const role = String(meJson?.role || '').toLowerCase();
            const expected = expectedRoleLabel(variant);
            ...
            await supabase.auth.signOut();
            toast.error('Failed to verify account role');
            setLoading(false);
            return;
          }
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                <Input
                  ...
                  placeholder={variant === 'organization' ? 'Enter your password' : 'Enter your password'}
                  value={password}
                  onChange={(e) => setPassword(e.target.value)}
                  required
                  className="h-12 bg-gray-50 border-gray-200 focus:border-amber-500 focus:ring-amber-500 pr-12 pl-3"
                />
                <Button
                  type="button"
                  variant="ghost"
                  ...
                  className="absolute inset-y-0 right-0 my-auto h-12 w-12"
